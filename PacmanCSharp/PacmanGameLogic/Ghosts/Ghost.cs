...
namespace Pacman.GameLogic.Ghosts
{
    ...
    public abstract class Ghost : Entity, ICloneable
    {
        ...
        public override void Move()
        {
            ...
            {
                if (waitToEnter > 0)
                { // going up and down before entering
                    if (Y < 113 || Y > 124)
                        waitToEnter--;
                    if (waitToEnter % 2 == 0)
                    {
                ...
                else if (waitToEnter == -2)
                { // moving up and entering
                    if (Y <= 93)
                    {
                        y = 93;
                        entered = true;
        ...
        protected void MoveMTS()
        {

            float distanceIfDirectionUp = (float)Math.Sqrt(Math.Pow(X - GameState.Pacman.X, 2) + Math.Pow((Y - 1) - GameState.Pacman.Y, 2));
            float distanceIfDirectionDown = (float)Math.Sqrt(Math.Pow(X - GameState.Pacman.X, 2) + Math.Pow((Y + 1) - GameState.Pacman.Y, 2));
            float distanceIfDirectionLeft = (float)Math.Sqrt(Math.Pow((X - 1) - GameState.Pacman.X, 2) + Math.Pow((Y) - GameState.Pacman.Y, 2));
            float distanceIfDirectionRight = (float)Math.Sqrt(Math.Pow((X + 1) - GameState.Pacman.X, 2) + Math.Pow((Y) - GameState.Pacman.Y, 2));

            float[] posDir = { distanceIfDirectionUp, distanceIfDirectionDown, distanceIfDirectionLeft, distanceIfDirectionRight };

            float[] posDirSorted = { distanceIfDirectionUp, distanceIfDirectionDown, distanceIfDirectionLeft, distanceIfDirectionRight };

            Array.Sort(posDirSorted);

            var posDirIntSorted = new List<int>();

            for (int i = 0; i < posDirSorted.Length; i++)
            {
                posDirIntSorted.Add(Array.IndexOf(posDir, posDirSorted[i]));
            }


            if (checkTwoConditionTogether(posDirIntSorted[0]))
            {
                NextDirection = (Direction)posDirIntSorted[0];
            }
            else if (checkTwoConditionTogether(posDirIntSorted[1]))
            {
                NextDirection = (Direction)posDirIntSorted[1];
            }
            else if (checkTwoConditionTogether(posDirIntSorted[2]))
            {
                NextDirection = (Direction)posDirIntSorted[2];
            }
            else if (checkTwoConditionTogether(posDirIntSorted[3]))
            {
                NextDirection = (Direction)posDirIntSorted[3];
            }
